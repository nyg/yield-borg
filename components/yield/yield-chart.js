...
const getStoredVisibility = () => {
   ...
   try {
      const stored = localStorage.getItem('yieldborg.visibility')
...
const saveVisibility = (visibility) => {
   localStorage.setItem('yieldborg.visibility', JSON.stringify(visibility))
}
...
export default function YieldChart({ settings, onVisibilityChange }) {
   ...
   const { data, error } = useSWR(`/api/yield?timeFrame=${timeFrame}`)
